...
namespace disassembler
{
	...
	// 
	OP_INFO* OP_TABLE = nullptr;

	void init()
	{
		OP_TABLE = new OP_INFO[N_X86_OPCODES]
		{
			{ "00", "add", { r_m8, r8 },					"Add" },
	...
	// Opens the disassembler in DLL mode
	// 
	void load()
	{
	...
	// or from a DLL
	// This is typically used to set the disassembler
	// to an external process (and its base module..
	// (use Psapi.h/TlHelp32.h to find the base module))
	// 
	...
	inst read(uintptr_t address)
	{
		...
		for (int opcode_at = 0; opcode_at < N_X86_OPCODES - 1; opcode_at++)
		{
			OP_INFO op_info = OP_TABLE[opcode_at];
	...
	std::vector<disassembler::inst> read_range(uintptr_t from, uintptr_t to)
	{
		uintptr_t at = from;
		auto inst_list = std::vector<disassembler::inst>();
